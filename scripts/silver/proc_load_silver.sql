...
BEGIN
		...
		CASE WHEN UPPER(TRIM(cst_marital_status)) = 'S' THEN 'Single'
			 ...
			 ELSE 'n/a'
		END cst_marital_status, -- Normaize martial states values to readable format
		CASE WHEN UPPER(TRIM(cst_gndr)) = 'F' THEN 'Female'
			 WHEN UPPER(TRIM(cst_gndr)) = 'M' THEN 'Male'
			 ELSE 'n/a'
		END cst_gndr, -- Normaize gender values to readable format
		cst_create_date
		FROM (
		...
		CASE WHEN sls_sales IS NULL OR sls_sales <= 0 OR sls_sales != sls_quantity * ABS(sls_price)
			 ...
			 ELSE sls_sales
		END AS sls_sales, -- Recalculate sales  original value is missing or incorrect
		sls_quantity,
